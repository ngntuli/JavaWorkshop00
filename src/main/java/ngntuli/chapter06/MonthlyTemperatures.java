...
public class MonthlyTemperatures {
	...
	static double max(double[][] temperature) {
		double result = temperature[0][0];
		for (int i = 0; i < temperature.length; i++) {
			for (int j = 0; j < temperature[i].length; j++) {
				double value = temperature[i][j];
				if (value > result) {
					result = value;
				}
			}
	...
	// method to display temperatures for a single day of each week requires a
	// single loop
